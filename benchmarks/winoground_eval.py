...
def run_winoground_experiment(mm, samples, order, system_prompt,
    ...
    for si, s in enumerate(samples):
        ...
        # LLaVA expands <image> inside generate(), so the prompt-slice lands in the
        # image region and decodes to empty; decode the full output and take the
        # text after the ASSISTANT tag (matches naturalbench_eval).
...
def main():
    ...
    for order in orders:
        ...
        o = meta["overall"]
        print(f"  [done] {order}: acc={o['acc']:.3f} group_acc={o['group_acc']:.3f} "
              f"f1={o['f1']:.3f}")
