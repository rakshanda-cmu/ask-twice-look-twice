...
from benchmarks.naturalbench_eval import _first_yes_no, YESNO_SUFFIX

MODEL_CHOICES = ["gemma-3-27b", "gemma-3-12b", "gemma-3-4b",
                 "qwen3-vl-8b", "qwen2.5-vl-7b", "internvl3-8b", "llava-1.5"]
...
def run_pope_experiment(mm, samples, order, system_prompt,
    ...
    for si, s in enumerate(samples):
        ...
        # LLaVA expands the single <image> placeholder to many tokens inside
        # generate(), so input_ids.shape[1] undercounts the prompt and the slice
        # lands in the image region (decodes to empty). Decode the full output and
        # take the text after the ASSISTANT tag instead (matches naturalbench_eval).
        if mm.model_name == "llava-1.5":
            raw = mm.tokenizer.batch_decode(out, skip_special_tokens=True)[0].strip()
            if "ASSISTANT:" in raw:
