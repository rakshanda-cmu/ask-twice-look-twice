...
def run_rf20_experiment(mm, samples, order, system_prompt,
    ...
    scales ONE of the two image occurrences by `echo_scale` (e.g. 0.25) while
    leaving the other at full resolution -- echo_which='first' scales the
    FIRST occurrence, 'second' scales the SECOND (echoed) occurrence.
    order_tag names the output file when it should differ from `order`
    (e.g. 'SITIT_echo2quarter'); defaults to `order`.
    """
    import torch
    ...
    if out_dir:
        ...
        if os.path.exists(results_path):
            try:
                ...
                print(f"  [resume] {tag}: {len(done)} samples already done")
            except Exception:
                records, done = [], set()
    ...
    for si, s in enumerate(samples):
        ...
        if (si + 1) % log_every == 0 or (si + 1) == n:
            ...
            print(f"  [{tag}] [{si+1}/{n}] acc={m['acc']:.3f} f1={m['f1']:.3f} "
...
def main():
    ...
    ap.add_argument("--echo-scale", type=float, default=None, dest="echo_scale",
                    help="Scale ONE of the two image occurrences (e.g. 0.25) in a "
    ...
    ap.add_argument("--tag-suffix", default="", dest="tag_suffix",
                    help="Suffix for the output filename, e.g. 'echo2quarter'.")
    args = ap.parse_args()
