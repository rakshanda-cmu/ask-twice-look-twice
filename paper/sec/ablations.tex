...
by $+0.21$ pair accuracy. Pushing to an eighth costs POPE a further $0.17$ and
leaves Winoground at its best value of the sweep.
...
RF20 is the exception, and it is the informative one. Its accuracy falls at
every step of the sweep ($-0.50$ F1 at half, $-1.76$ at quarter, $-1.88$ at an
eighth).
...
without costing accuracy, except where the evidence is small. Only the
\emph{second} image occurrence is downscaled; the first stays at full
resolution. Bold marks the best echo scale in each row. Metrics are F1 for POPE
and the RF20 presence task, group accuracy for Winoground and pair accuracy for
NaturalBench. RF20 is the one benchmark that degrades monotonically at every
step, $80.56 \to 80.06 \to 78.80 \to 78.68$.}
...
cheapest echo is the one to ship. Cost against accuracy on NaturalBench; the
$x$ axis is extra tokens per query relative to question-first, on a symmetric
log scale so that $0$ can be shown. The dashed line is the question-last
baseline, which every image echo clears. The $\tfrac{1}{8}$-resolution echo
($+107$ tokens) is statistically indistinguishable from the full-resolution one
($+3{,}352$).}
\label{fig:pareto}
\end{minipage}
\end{figure}
