\section{Full ordering ladder across 13 benchmarks}
\label{app:breadth}

\begin{table}[h]
\caption{Full ordering ladder for Qwen3-VL-8B across 13 benchmarks (video QA
below the rule). Best of the four orderings in bold. The echo column gives
\sitit{} with the second image copy at half resolution. Question-first costs
$7.01$ points on average; image echoing recovers all of it and adds $1.18$.}
\label{tab:breadth}
\begin{center}
\small
...
\midrule
\multicolumn{2}{l}{\emph{Mean over 13}} & & & & & & $\mathbf{-7.01}$ & $\mathbf{+1.18}$ \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
$P(\text{correct})$ over the $\{$Yes, No$\}$ decision, layers 26 to 35, on the
$300$-pair disagreement set:
...
Both orderings sit at $\approx 0.50$ through layer 25.
