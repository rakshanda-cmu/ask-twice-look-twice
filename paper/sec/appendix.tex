...
\begin{table}[h]
\caption{Asked as a yes/no question rather than as detection, the same RF20
images give the opposite result: question-first hurts on 7 of 7 categories.
Accuracy by category, Qwen3-VL-8B, $24{,}449$ questions. Bold marks the best
ordering in each row. Compare Table~\ref{tab:rf20map}, where the sign flips.}
\label{tab:rf20}
\begin{center}
\small
...
\end{center}
\caption{Each ordering answers through a different path, and cutting the other
one costs it nothing. Bars are the change in accuracy against the unmodified
run ($57.6$ for question-first, $58.8$ for question-last), so downward is
damage. Severing answer-to-question hurts only question-last; severing
answer-to-image hurts only question-first; severing a random pre-image span of
equal length, the control for simply removing attention mass, moves both by at
most $0.8$ points.}
\label{fig:knockout}
\end{figure}
...
\begin{table}[h]
\caption{Four of five model families lose accuracy when the question moves
before the image, so the paradox is not one model's quirk. NaturalBench,
$1{,}900$ groups. Bold marks the best ordering in each row; n/a marks
combinations that were not run. LLaVA-1.5's question-first group accuracy is
exactly zero, meaning it never gets all four items of any group right.}
\label{tab:models-nb}
\begin{center}
\small
...
\begin{table}[h]
\caption{The repair outlives the diagnosis: neither Gemma model shows the
paradox, yet image echoing is still their best ordering on 7 of 8
model-benchmark pairs. The four benchmarks run for all three models. Bold marks
the best ordering in each row; the mean rows average the $\Delta$ columns above
them.}
...
\begin{table}[h]
\caption{The paradox is significant wherever it appears, and its absence in
Gemma is also clear rather than merely underpowered. Consecutive rungs of the
ladder plus the headline comparisons, paired on group or example index.
$\Delta$ is B minus A with a paired bootstrap 95\% interval; $p$ is an exact
two-sided McNemar test. \sti{} versus \sit{} is highly significant for
Qwen3-VL-8B on both benchmarks and absent for Gemma-3-27B.}
...
\end{center}
\caption{Question-first answers by reading the image instead of the question,
and the two orderings only part company in the last quarter of the network.
Measured on the 150 NaturalBench pairs where question-first is wrong and
question-last is right. \textbf{(a)} Attention mass leaving the answer position,
to the question (solid lines) and to the image (dashed): at layer 23, where
question attention peaks for both, question-first sends $2.19\times$ less of it
to the question. \textbf{(b)} Probability of the correct answer at each layer.
Both baselines sit at chance through layer 25 (shaded), fork at layer 26 and
never re-cross; both echo orderings track question-last, not question-first.}
\label{fig:mechanism}
\end{figure}
...
\begin{table}[h]
\caption{Question-first reads the image where question-last reads the question,
and the ratio between the two inverts. Read-out probe on the disagreement set;
$a_q$ and $a_{img}$ are attention mass from the answer position onto the
question and image spans, and the layer of each peak is in brackets.}
\label{tab:probe}
\begin{center}
...
\begin{table}[h]
\caption{The question rewrites the image tokens themselves, and a later copy of
it provably cannot. Mean cosine between each ordering's layer-18 image tokens
and a forward pass on the image alone, so $1.0$ means untouched. \ist{} is that
baseline by construction; \stit{} is identical to \sti{} to five decimals
because causal masking prevents a later question copy from reaching earlier
patches.}
\label{tab:cosine}
\begin{center}
\begin{tabular}{lrrrr}
...
\begin{table}[h]
\caption{Repeating the question is nearly free; repeating the image is not, but
its price falls with echo area because vision tokens scale with pixels. Exact
counts per query, mean over 20 images spanning RF20's constituent datasets,
Qwen3-VL-8B. $\Delta$ is against the \sti{} baseline, which is why that row
reads n/a. \sti{} and \sit{} contain the same tokens in a different order, so
their totals are identical and only \sti{} is listed.}
\label{tab:cost}
\begin{center}
...
optimized prompt's, with the change in parentheses. This is the table
Section~\ref{sec:gepa} reads its numbers from. ``Gap closed'' is the
...
\begin{table}[h]
\caption{GEPA's cost is dominated by a one-time search, not by inference.
Search budget under the \sti{} ordering; the recurring per-query cost is the
prompt-length delta in Table~\ref{tab:gepa-tokens}.}
\label{tab:gepa-cost}
\begin{center}
\begin{tabular}{lrrrrr}
\toprule
...
\begin{table}[h]
\caption{The optimized prompts are cheap to run, and under question-last the
optimizer had nothing to add. Prompt length of GEPA's system messages in tokens
per query. A zero delta means the optimizer returned the seed prompt unchanged,
which is what happened under \sit{} on POPE and VQAv2.}
...
Section~\ref{sec:anatomy-readout}, where the two orderings are indistinguishable
