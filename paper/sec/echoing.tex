...
not reliably better than asking the question last (NaturalBench $35.00$ against
$35.05$, $p = 1.00$), so its role is to repair question-first prompting, not to
improve on question-last. Image echoing is the real gain and the claim we defend
($+0.085$ on Winoground, $p = 2.4\times10^{-5}$), though on POPE it does not
help.

Prior work left open what the echo is made of and what it costs.
Sections~\ref{sec:results} and~\ref{sec:ablations} take up that thread.
