...
Qwen3-VL-8B is the primary model and carries every analysis; for breadth we also
run Gemma-3-27B, Gemma-4-31B, Qwen2.5-VL-7B, InternVL3-8B and LLaVA-1.5. All
results use greedy decoding with a fixed answer-token budget, and every ordering
of a benchmark shares one system message, one question string per item, one
...
Section~\ref{sec:results} adds fourteen not tested in prior work: RF20-VL, used
both as a detection benchmark scored by mAP and as a $24{,}449$-question yes/no
presence task over the same images; nine image benchmarks (BLINK
\citep{blink2024}, CV-Bench \citep{cambrian2024}, HR-Bench, MMStar
