...
Amber boxes mark three individual patches in the question-last panel, joined by
a dashed arrow to the same patch in the question-first panel, where it is boxed
in red: \emph{running} becomes \emph{chase}, \emph{boy} becomes \emph{ahead},
and \emph{baseball} becomes \emph{child}. Echoing the question keeps all 12
steered patches and answers correctly again.}
\label{fig:steering}
\end{figure}

\subsection{Locating the rewrite automatically}
...
take the top-1 token per patch. Patches whose token is unchanged are dropped.
Of those that remain, we mark the ones whose new token belongs to the
question's vocabulary, matched by stem against the question's content words and
their near variants, so that a patch turning \emph{throws} into \emph{chase}
counts and one turning \emph{lush} into \emph{grass} does not. The result is a
count with a denominator, reported in every figure caption, rather than a
gallery.
