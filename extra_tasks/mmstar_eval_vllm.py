"""
MMStar ordering ablation via the vLLM library.

MMStar (Chen et al. 2024) is 1,500 hand-curated multiple-choice VQA samples
selected for genuine visual dependency (not answerable from text priors or
world knowledge alone) and no train/test leakage, spanning 6 core
capabilities (coarse/fine perception, instance reasoning, logical reasoning,
math, science & technology).

Source: Lin-Chen/MMStar (HF), mmstar.parquet. "question" already embeds the
lettered options; "answer" is the bare gold letter.
Metric: multiple-choice exact match on the "(X)" letter, pooled + per-category.

Run:
  HF_HOME=/data2/hf_cache CUDA_VISIBLE_DEVICES=0,1 \
    /home/grg/anaconda3/envs/qwen-vllm-env/bin/python extra_tasks/mmstar_eval_vllm.py \
...
def load_mmstar():
    ...
    p = hf_hub_download("Lin-Chen/MMStar", "mmstar.parquet", repo_type="dataset")
    df = pd.read_parquet(p)
    rows = []
    for _, r in df.iterrows():
        rows.append({"index": int(r["index"]), "question": r["question"],
                    "answer": str(r["answer"]).strip(), "category": r["category"],
                    "l2_category": r["l2_category"], "image_bytes": r["image"]})
    return rows
...
def run_order(llm, sp, tag, letters, rows, log_every, echo_scale=None, echo_which=None):
    out = os.path.join(OUT_DIR, f"mmstar_order-{tag}_{MODEL_TAG}.json")
    ...
    for o, r in zip(outputs, rows):
        ...
        results.append({"index": r["index"], "category": r["category"],
                        "l2_category": r["l2_category"], "gt": gt, "pred": pred,
    ...
    by_cat_summary = {c: {"n": v["n"], "accuracy": v["correct"] / v["n"]}
                      for c, v in by_cat.items()}
    meta = {"benchmark": "MMStar", "ordering": tag, "model": MODEL_TAG,
            "engine": "vllm", "n": len(results),
            "accuracy": n_correct / max(1, len(results)), "by_category": by_cat_summary,
...
def main():
    ...
    rows = load_mmstar()
    print(f"[data] {len(rows)} MMStar rows", flush=True)
    ...
    for tag in [o.strip() for o in args.orders.split(",") if o.strip()]:
        ...
        if args.echo_scale is not None:
            ...
            out_tag_name = tag
        print(f"=== MMStar · ordering {out_tag_name} ===", flush=True)
