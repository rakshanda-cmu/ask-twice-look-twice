...
Run:
  HF_HOME=hf_cache CUDA_VISIBLE_DEVICES=0,1 \
    python extra_tasks/mvbench_eval_vllm.py \
...
VIDEO_ROOT = "hf_cache/newtasks/mvbench/videos"
IMG_CAP = 512
...
def run_order(llm, sp, tag, letters, rows, k_frames, model_tag, log_every,
    ...
    for r in rows:
        ...
        if echo_scale is not None:
            ...
            convs.append(build_conversation(letters, task_text, None,
...
def main():
    ...
    for tag in [o.strip() for o in args.orders.split(",") if o.strip()]:
        ...
        if args.echo_scale is not None:
            ...
            out_tag_name = tag
        print(f"=== MVBench · ordering {out_tag_name} ===", flush=True)
