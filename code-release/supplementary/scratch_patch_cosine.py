...
def main():
    ...
    base_cache = {}
    orders = ["IST", "STI", "STIT"] if args.validate else ["STI"]
    acc = {o: None for o in orders}
    cnt = {o: 0 for o in orders}
    ident = {"cos(base,IST)": [], "cos(STI,STIT)": []}
    grid = None
    t0 = time.time()
    ...
    for pi, (imgrel, question) in enumerate(pairs):
        ...
        if args.validate and "IST" in per:
            ident["cos(base,IST)"].append(float(per["IST"][1].mean()))
