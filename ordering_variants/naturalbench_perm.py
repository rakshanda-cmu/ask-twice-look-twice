"""
SITIT-with-randomly-permuted-2nd-image control on NaturalBench (Qwen3-VL-8B) — SEPARATE,
...
from benchmarks.naturalbench_eval import (
...
def _build_meta(model_name, records, max_tokens):
    return {
        ...
        "cue_mode": False, "think": False, "permute_second_image": True,
        "base_order": ORDER,
...
def main():
    ...
    print(f"[data] SITIT-permute (2nd image randomly reordered) over {len(groups)} groups")
    mm = ModelManager(args.model)
    install_permute_hooks(mm)
    PERMUTE["on"] = True
