"""
Winoground SITIT-with-permuted-2nd-image (S·I·T·Ī·T) — Qwen3-VL-8B or Gemma-3-27B.
Runs the SITIT ordering with the 2nd image block permuted (shared permute_image_hooks),
writes standard Winoground result files tagged SITIT_rev. Additive; nothing changed.

    CUDA_VISIBLE_DEVICES=0 python winoground_sitit_permute.py --model qwen3-vl-8b
"""
import argparse, json, os, time
import torch
from core.constants import SYSTEM_MESSAGE
from benchmarks.winoground_eval import load_winoground_samples, metrics, _pred_yes_no, _question
from core.permute_image_hooks import install_permute_hooks, PERMUTE

TAG, ORDER = "SITIT_perm", "SITIT"


def _meta(model, records, max_tokens):
    return {"model": model, "order": TAG, "order_tag": TAG, "base_order": ORDER,
            "permute_second_image": True, "system_prompt": SYSTEM_MESSAGE,
...
def main():
    ...
    print(f"[data] {len(samples)} Winoground pairs · SITIT-permute · {args.model}", flush=True)
    mm = ModelManager(args.model)
    install_permute_hooks(mm); PERMUTE["on"] = True
